import { IProducer, IEosIoChainGetGlobal, IEosIoChainGetProducerJson, IEosIoChainGetProducers } from "../types";
...
export const getClaimRewards = (producer: IEosIoChainGetProducers, global: IEosIoChainGetGlobal, rank: number) => {
...
export const getProducers = (
  ...
  producerJson: IEosIoChainGetProducerJson[],
  ...
  const bpList: IProducer[] = producers.map((producer, index) => {
    ...
    const { claimRewardsTotal, claimRewardsUnreceived } = getClaimRewards(producer, global, index + 1);
    const weight_percent = (parseInt(producer.total_votes) / parseInt(global.total_producer_vote_weight)) * 100;
    let urlFull = "";
    let urlSimple = "";
    let candidate_name = "";
    let logo = "";
