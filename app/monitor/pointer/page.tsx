"use client"; // Required for hooks like usePointer, useState and client-side interactions

import { useState } from "react";
import Link from "next/link";
import { usePointer } from "../../../hooks"; // Adjust path as needed
import { formatNumber } from "../../../utils"; // Adjust path as needed
import { EndPointStatus } from "../../../enums"; // Adjust path as needed

export const metadata = {
  title: "节点监控 | FIBOS 导航", // This title was on the Layout, might need to be more specific
};
...
export default function MonitorPointerPage() {
  const { accessPoints, points } = usePointer();
  const [isOpen, setIsOpen] = useState(false);

  return (
    // <Layout title="节点监控 | FIBOS 导航"> // Layout is handled by app/layout.tsx
    <main>
      <div className="px-6">
        <div className="pb-4">
          <Link
            href="/monitor"
            className="rounded bg-white py-2 px-4 text-indigo-500 transition duration-150 ease-in-out hover:bg-indigo-500 hover:text-white"
          >
            出块节点在线状态
          </Link>
          <Link
            href="/monitor/pointer"
            className="ml-4 cursor-default rounded bg-indigo-500 py-2 px-4 text-white transition duration-150 ease-in-out"
          >
            接入点状态监测
          </Link>
        </div>
