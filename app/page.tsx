"use client"; // This is required if we use hooks like useSites

import { useSites } from "../hooks"; // Assuming hooks are adjusted or still work
                ...
                <div key={item.name} className="relative sm:flex">
                  ...
                  <ul className="flex flex-wrap pt-4 sm:gap-x-4">
                    {item.child.map((item) => (
                      <li
                        key={item.name}
                        className="relative w-1/2 border-r border-b border-dotted odd:last:w-full odd:last:border-r-0 even:border-r-0 sm:w-64 sm:border-b-0 sm:pr-4 sm:odd:last:w-64 sm:odd:last:border-r sm:even:border-r"
                      >
                        <a
                          href={item.url}
                          target="_blank"
                        ...
                        {item.more && (
                          <p className="absolute top-0 right-0 mt-2 mr-2 sm:mr-6">
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="block rounded bg-blue-600 py-1 px-2 text-xs text-white"
                            >
                              {item.more.name}
                            </a>
                          </p>
