              ...
              <tbody>
                {bpList.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td className="px-2 py-2 text-center">{item.logo && <span className="mx-auto block h-12 w-12 bg-cover" style={{ backgroundImage: "url(" + item.logo + ")" }}></span>}</td>
                    <td>
                    ...
                    <td>
                      {
                        {
                          active: <span className="inline-flex rounded-full bg-green-100 px-2 text-xs font-semibold text-green-800">当选节点</span>,
                          waiting: <span className="inline-flex rounded-full bg-slate-100 px-2 text-xs font-semibold text-slate-800">待机节点</span>,
                    ...
                    <td className={"" + (item.claimRewardsUnreceived ? "font-bold text-green-500" : "text-slate-400")}>{formatNumber(item.claimRewardsUnreceived)} FO</td>
                    <td className="text-slate-400">
                      <a href={item.urlFull} target="_blank" rel="noreferrer" className="text-indigo-500 transition duration-150 ease-in-out hover:text-indigo-800">
                        {item.urlSimple}
                      </a>
                    </td>
                    <td>
                      <Link href={"/producer/detail?account=" + item.owner} className="text-indigo-500 transition duration-150 ease-in-out hover:text-indigo-800">
                        详情
                      </Link>
                    </td>
                  </tr>
